#!/usr/bin/env python3
"""v225 CLAUDE: fix the U bias where the game actually applies it.

v222 and v224 both stopped the scenery bleed -- the placement is right -- but
left blank cells, because the drawn packets kept reading U base 0 while the
upload wrote x999.  Disassembling the running RAM out of the savestates shows
the patched instruction was in place:

    801A2074  addiu a3, a3, 0x9c      the value went in fine

and the packets still came out at U 0.  0x801A2060 is a hook-side branch, not
the path the glyph packets take.  The stock builder does it here:

    8016B5A0  addu  v0, v0, t0
    8016B5A4  sll   v0, v0, 2
    8016B5A8  sb    v0, 0x29(a1)      V -- already lands on 164 via index base
    8016B5AC  lbu   v1, 0xd(a2)
    8016B5B4  bne   v1, 6, skip       is this a cache glyph
    8016B5BC  lbu   v0, 0x28(a1)
    8016B5C4  addiu v0, v0, 4         the U bias that actually reaches VRAM
    8016B5C8  sb    v0, 0x28(a1)

Measured across 168 older savestates the packets read U 4,16,28,40,52,64,76 --
exactly 4 + cell*12 -- which is this instruction and not the hook one.

So v225 is v224 plus one more word: the stock U bias 4 -> 156, matching the
upload that already moved to x999.
"""
...
OUT_STEM = "arc1_v225_CLAUDE_stock_u_bias_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v225_CLAUDE_stock_u_bias"

PSX = "PSX.EXE"
OLD_ROW, NEW_ROW = 40, 35
OLD_Y, NEW_Y = 480, 420
OLD_V, NEW_V = 224, 164
OLD_X, NEW_X = 961, 999
OLD_U, NEW_U = 4, 156
...
def main() -> None:
    ...
    I = old.i_type
    # the low-helper test at 0x801A2060 stays at 40: it detects cache glyphs,
    # it is not a coordinate.  v222 moved it and the U fix-up stopped running.
    edits = []
    ...
    # the U bias the glyph packets really use, in stock code
    STOCK_U_BIAS = 0x8016B5C4
    su_at = old.file_at(STOCK_U_BIAS)
    su_want, su_new = I(0x09, v171.V0, v171.V0, OLD_U), I(0x09, v171.V0, v171.V0, NEW_U)
    su_actual = struct.unpack_from("<I", exe, su_at)[0]
    if su_actual != su_want:
        raise SystemExit(f"stock U bias: 0x{su_actual:08X}, expected 0x{su_want:08X}")
    struct.pack_into("<I", exe, su_at, su_new)
    edits.append(("stock U bias", su_at, su_want, su_new))
    ...
    report = [
        "v225 CLAUDE TEST ONLY - v224 plus the stock U bias at 0x8016B5C4",
        ...
        "low_helper_detect=40 unchanged", f"cache_index_base={OLD_ROW}*84->{NEW_ROW}*84", f"cache_y={OLD_Y}->{NEW_Y}",
        ...
        "expected=no scenery bleed AND packet U base 156 so cached glyphs render",
    ]
