...
BASE_PATCH = ROOT / "99_backup" / "story_test_14_s1011_two_blocks_success.zip"
WORK = ROOT / "01_work" / "story_test_15_s1011_three_blocks"
WORK_FONT = WORK / "COMM.IMG"
WORK_S1071 = WORK / "1" / "S1071.DAT"
WORK_S1011 = WORK / "1" / "S1011.DAT"
OUTPUT = ROOT / "03_output" / "story_test_15_s1011_three_blocks_patch_only.zip"
EXPECTED_BASE_HASH = "D7A1A14BBE2138F0FD335FE2A093A6E42135448CEA2D6317FE38486AE932AB4B"
FILLER = 0x9C
...
def write_text_files() -> None:
    test_info = """Arc the Lad 1 JP Korean Patch - Story Test 15 S1011 three blocks

Purpose:
- Extend S1011 from two translated blocks to three translated blocks.
- Keep COMM.IMG and S1071.DAT unchanged from story_test_14.

Change:
- First block remains 아크여 / 마을로 가라.
- Second block remains 이 뒤로 / 가라.
- Third S1011 block at 0x47954 becomes:
  조심하라

Expected DuckStation check:
- First two S1011 blocks remain unchanged.
- Third S1011 block should display 조심하라.
- No freeze after the third block.
"""
    charmap = """Story test 15 S1011 three-block map

Same character map as story_test_14.
...
def main() -> None:
    if digest(BASE_PATCH) != EXPECTED_BASE_HASH:
        raise SystemExit("story_test_14 success artifact hash mismatch")
    if WORK_FONT.exists() or WORK_S1071.exists() or WORK_S1011.exists() or OUTPUT.exists():
        raise SystemExit("Story test 15 already exists; refusing to overwrite it.")
    ...
    patch_block(s1011, 0x47954, 21, 0x47969, ["조심하라"])
