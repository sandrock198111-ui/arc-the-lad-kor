...
OUT = ROOT / "03_output"
STEM = "arc1_v301_ui_align_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v301_ui_align"
...
def main() -> None:
    ...
    # v197 did not squeeze Korean into each original slot -- it repacked the
    # whole UI string block and updated the pointers.  0x80224..0x82420 holds
    # 612 strings reached by pointers; rewriting them in order gives every
    # string as much room as it needs, bounded only by the block total.
    BLK_LO, BLK_HI = 0x80224, 0x82420
    ptr_of = collections.defaultdict(list)
    for at in range(0, len(exe) - 4, 4):
        val = struct.unpack_from("<I", exe, at)[0]
        a = val - R2F
        if BLK_LO <= a < BLK_HI:
            ptr_of[a].append(at)
    order = sorted(ptr_of)
    # The ui_* tables were built against a different generation and their
    # string_offset sits 2-4 bytes off the real boundary.  Pointer targets are
    # authoritative, so attach each translation to the target that owns it.
    owner = {}
    for a in ui:
        cands = [x for x in order if x <= a]
        if cands:
            owner.setdefault(cands[-1], ui[a][1])
    attached = sum(1 for x in order if x in owner)
    step(f"UI 블록 문자열 {len(order)}곳,  포인터 "
         f"{sum(len(v) for v in ptr_of.values())}개,  번역 연결 {attached}곳")
    ...
    payload, newpos, ui_ok, ui_keep = bytearray(), {}, 0, 0
    for a in order:
        t = owner.get(a)
        code = None
        if t:
    ...
    step(f"UI 재배치 {len(order)}곳 (한글 {ui_ok}, 원본 유지 {ui_keep}),  "
    ...
    tmp.replace(out)
    rep = [
        "v301 TEST ONLY - UI repack, translations matched to pointer targets",
