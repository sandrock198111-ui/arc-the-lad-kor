"""v137: choice bodies, keeping the menu geometry and redirecting what will not fit.

The rule this build follows was written down on 2026-07-17 and I had not read it:

    The raw byte positions of every E5 and E6 control are part of the menu geometry
    and can determine cursor placement and branch behaviour. The safe reconstruction
    model is source geometry plus Korean text: begin with the clean original body,
    keep every E5/E6 at its original raw offset, place fitting options inline, and
    use an external E2 for the rest.

    Every body has two option spans with source capacities 2 and 7 bytes. The first
    span cannot hold a natural Korean affirmative inline, but each span can hold a
    two-byte E2 redirect.

That last sentence is the part that matters and the part v135 missed. A span too small
for the Korean is not a span that needs the Korean shortened -- it needs two bytes for
an E2 and a slot to point at. 병사 in three bytes, 출발하시겠습니까? in nine: both fit,
because neither is written where it appears.

So v135 was right to refuse text that moves the markers -- v0.21 moved them in 243
bodies and the Choppin blank-option bug at `21/S2042.DAT 0x47FF0` is the recorded
result -- but wrong to leave those bodies in Japanese instead of redirecting them.

Each redirected span gets `E2 <disk id>` at its start, the rest of the span filled with
the space glyph, and `span length - 2` in the slot's metadata byte so the renderer
resumes exactly at the span's own marker. A span never contains an E6, so the rule
against skipping over a line break does not apply here.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v136_scene_restore_4CE9EE1E.zip"
BASE_SHA = "4CE9EE1E63DC8CE320D90FCA74EDFA103EB97BCC6AC0E9503D773FD855E91F6A"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v137_choices_by_geometry"
ANALYSIS = ROOT / "01_work/analysis/arc1_v137_choices_by_geometry"
LINEBREAK = b"\xE6\x01"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v136")
    ...
    lines = [
        "v137 choice bodies written to the source geometry",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(members)}; PSX.EXE and COMM.IMG byte-identical to v136",
        "",
