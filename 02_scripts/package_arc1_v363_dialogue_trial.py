"""Fresh original-disc staging for the V363 player-review trial."""
import json,subprocess,sys
import package_test_iso as p
import build_arc1_v363_dialogue_trial as b
ROOT=b.ROOT;stem='V363_DIALOGUE_TRIAL_TEST'
p.WORK=ROOT/'01_work/package_v363_dialogue_trial';p.FILES=p.WORK/'files';p.STATE=p.WORK/'applied.json'
...
assert b.digest(b.OUT.read_bytes())==report['zip_sha256']==verification['zip_sha256']
assert verification['static_pass']
...
result=subprocess.run([sys.executable,'-X','utf8',str(ROOT/'02_scripts/verify_iso_layout.py'),str(binary),str(b.OUT)],cwd=ROOT,check=True,capture_output=True,text=True,encoding='utf-8')
assert binary.name in cue.read_text(encoding='utf-8-sig')
package={'bin':str(binary),'cue':str(cue),'bin_sha256':p.digest(binary),'cue_sha256':p.digest(cue),'archive_sha256':report['zip_sha256'],'verification':result.stdout,'runtime_verified':False}
(b.AN/'package.json').write_text(json.dumps(package,ensure_ascii=False,indent=2),encoding='utf-8')
print(result.stdout);print('BIN',package['bin_sha256']);print('CUE',cue)
