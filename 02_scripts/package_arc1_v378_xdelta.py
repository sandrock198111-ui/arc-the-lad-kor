"""Verified original Rev1-to-V378 hotfix package, without game images."""
import json,os,subprocess,tempfile
from pathlib import Path
from zipfile import ZipFile,ZipInfo,ZIP_DEFLATED
from package_arc1_v375_xdelta import hashes,TOOL,SOURCE,ROOT,TOOL_SHA
import build_arc1_v378_followup as build
TARGET=ROOT/'03_output/V378_FOLLOWUP.bin'
OUT=ROOT/'03_output/arc1_kor_v0.9.4_rev1'
BUNDLE=ROOT/'03_output/arc1_kor_v0.9.4_rev1.zip'
...
def main():
    ...
    assert report['cpu']['water_temple']['return_value']==1 and len(report['applied'])==8
    work=Path(tempfile.mkdtemp(prefix='v378_xdelta_',dir=ROOT/'01_work'))
    patch=work/'arc1_kor_v0.9.4_rev1.xdelta';decoded=work/'verified.bin';repeat=work/'repeat.xdelta'
    ...
    verification=dict(version='v0.9.4',internal_build=378,source=src,target=tgt,patch=hashes(patch),
        ...
        inherited_v377_ui_edits=4,inherited_v377_dialogue_edits=2,
        v378_dialogue_edits=8,v378_cursor_operands=1,captured_cpu_cases=4,
    ...
    readme=f'''아크더래드 1 한글 패치 v0.9.4 (V378 긴급 수정) — 일본판 Rev 1 전용

적용 방법
1. 일본판 Rev 1 원본 BIN에 동봉 xdelta를 적용하세요.
   기존 v0.9.1/v0.9.2/v0.9.3/한글패치 BIN에 덧씌우지 마세요.
2. 출력 이름을 arc1_kor_v0.9.4.bin으로 지정하고 동봉 CUE를 옆에 두세요.
...
수정 내용
- V377에 누적 적용한 V378입니다. 제보 상태저장의 V376으로 돌아가지 않았습니다.
- '난 언제든 좋아'는 원문도 짧은 대사임을 확인해 유지했습니다.
- 쿠쿠루의 '맡겨 두게'를 '나한테 맡겨!'로 수정했습니다.
- 제보된 두 확인창 선택지를 '좋아 / 역시 그만둘래'로 수정했습니다.
- 여섯 맵의 비단 허리띠 안내에서 여는 괄호 앞을 줄바꿈했습니다.
- SC081의 제보된 선택 커서만 한 줄 아래로 이동했습니다. 선택문은 유지했습니다.

아래 V377 및 V376 수정도 모두 포함합니다.
...
'''
    files={patch.name:patch.read_bytes(),'arc1_kor_v0.9.4.cue':b'FILE "arc1_kor_v0.9.4.bin" BINARY\r\n  TRACK 01 MODE2/2352\r\n    INDEX 01 00:00:00\r\n',
