"""v134: the ten lines that overflowed the dialogue window, shortened.

Save state 5 froze on `21/S2041.DAT 0x485A6` in v133, the same line that froze save
state 6 two builds ago: 702 pixels of Korean in a window that holds 3 rows of 228. Ten
lines were over that budget, six of them the same 초핀 line about summons repeated
across six scenes, so one rewording covered six of the ten. Each was shortened until it
fits, keeping the Japanese's own content -- `게임이` and `사람의` stay in the ship line
because `ゲームが` and `人の` are there.

Nothing else changed: same in-place text pass, same guards.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v133_text_with_r_1FE292AB.zip"
BASE_SHA = "1FE292ABB09B2688ECDA1AB56864DE54E5E3E59E522B538DB027BDD59DF9072A"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v134_window_fit"
ANALYSIS = ROOT / "01_work/analysis/arc1_v134_window_fit"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v133")
    ...
    lines = [
        "v134 the ten overflowing lines, shortened",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(rebuilt)}; PSX.EXE and COMM.IMG byte-identical to v133",
        "",
