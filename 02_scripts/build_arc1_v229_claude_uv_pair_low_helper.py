#!/usr/bin/env python3
"""v229 CLAUDE: apply the UV pair move where the bias actually runs.

v228 uploaded the glyphs to the new rectangle correctly -- the savestates show
230 of 252 halfwords at x999,y420 and nothing left at x961,y480 -- but the
packets still read U 4,16,28 and V 224.  That U is the stock +4 bias, so it was
still being applied, which means the site v228 rewrote (0x8016B5C4, inside the
stock packet builder) is not the one that executes.  The bias comes from the low
helper instead:

    801A2060  addiu a3, t0, -0x28    row == 40, i.e. a cache glyph
    801A2064  bnez  a3, skip
    801A206C  lbu   a3, 0x28(a1)     U
    801A2070  nop
    801A2074  addiu a3, a3, 4        the bias that reaches VRAM
    801A2078  sb    a3, 0x28(a1)

v222 did edit this instruction, and U came out 0 -- because it also moved the
test on the first line to 35, killing the whole block.  With the test untouched
the block runs, which is exactly what v228 just demonstrated.

0x28 and 0x29 are adjacent, so the same halfword trick fits here without adding
an instruction:

    lhu   a3, 0x28(a1)
    nop
    addiu a3, a3, -15204     +156 to U, -60 to V
    sh    a3, 0x28(a1)

U becomes col*12 + 156 (x999..1017) and V becomes 164 (y420), matching the
upload rectangle v228 already proved correct.  The atlas row, the index base and
the cache-detect test all stay as v210 had them, so only cache glyphs move.
"""
...
OUT_STEM = "arc1_v229_CLAUDE_uv_pair_low_helper_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v229_CLAUDE_uv_pair_low_helper"
...
def main() -> None:
    ...
    for site, want, fresh in (
        (0x801A206C, (0x24 << 26) | (A1 << 21) | (A3 << 16) | 0x28,      # lbu -> lhu
                     (0x25 << 26) | (A1 << 21) | (A3 << 16) | 0x28),
        (0x801A2074, I(0x09, A3, A3, OLD_U), I(0x09, A3, A3, DELTA - 0x10000)),
        (0x801A2078, (0x28 << 26) | (A1 << 21) | (A3 << 16) | 0x28,      # sb -> sh
                     (0x29 << 26) | (A1 << 21) | (A3 << 16) | 0x28),
    ):
    ...
    report = [
        "v229 CLAUDE TEST ONLY - UV halfword move in the low helper, the site that runs",
        ...
        "expected=packets read U156+ V164; upload already verified at x999,y420",
    ]
