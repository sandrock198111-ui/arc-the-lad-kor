...
STEM = "arc1_v264_decoded_static_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v264_decoded_static"
...
def index_of(tok: bytes, exe: bytes) -> int | None:
    ...
    if lead in (0xE9, 0xEA):
        ...
        if not 0 <= slot < SLOTS:
            return None
        v = tget(exe, slot)
        return v if v < STATIC_MAX else None       # >= 0x600 is a cache request
...
def main() -> None:
    ...
    table_cells = {tget(bytes(exe), s) for s in range(SLOTS)
                   if tget(bytes(exe), s) < STATIC_MAX}
    ...
    todo = [s for s in range(SLOTS) if tget(bytes(exe), s) >= STATIC_MAX]
    placed, missing = [], 0
    for slot in todo:
    ...
    for i in sorted(inked):
        if read(bytes(font), i) != read(before[COMM], i):
            raise SystemExit(f"overwrote a cell that had a picture: {i}")
    ...
    rep = [
        "v264 TEST ONLY - all cached glyphs made resident from decoded pixels",
        ...
        f"free cells below {STATIC_MAX} left={len(free)}",
        "occupancy resolved through 0x801A7520, the table the game reads",
        "verified: no cell that already held a picture was touched",
