...
BASE = ROOT / "03_output" / "story_sf0b1_return_cursor_fixed_full_patch_only.zip"
SOURCE = ROOT / "01_work" / "4" / "S4041.DAT"
MANIFEST = ROOT / "05_docs" / "story_s4041_bulk_translation.csv"
EXTENDED_CHARMAP = ROOT / "05_docs" / "korean_charmap_extended.csv"
OUTPUT = ROOT / "03_output" / "story_bulk_s4041_cursor_fixed_full_patch_only.zip"
TARGET = "4/S4041.DAT"
LINEBREAK = b"\xE6\x01"
BASELINE_DIRS = (
...
def verify_codes_unused(rows: list[dict[str, str]]) -> None:
    ...
    for code, char in codes.items():
        if counts[code]:
            raise SystemExit(f"extended code {code.hex()} for {char} occurs in story data")
...
def cursor_bytes(font: bytes) -> bytes:
    # COMM logical rectangle x=0..31, y=128..159; two pixels per byte.
    row_bytes = 0x380
...
def main() -> None:
    ...
    verify_codes_unused(extended_rows)

    with zipfile.ZipFile(BASE) as archive:
        files = {info.filename: archive.read(info.filename) for info in archive.infolist()}
    if len(files) != 30 or FONT_TARGET not in files or TARGET in files:
    ...
    with zipfile.ZipFile(BASE) as base, zipfile.ZipFile(OUTPUT) as output:
        ...
        if len(names) != 31 or len(names) != len(set(names)):
            raise SystemExit("output must contain 31 unique game files")
    ...
    print("\n".join(report))
    print(f"entries=31 font_changed_bytes={sum(a != b for a, b in zip(base_font, font))}")
