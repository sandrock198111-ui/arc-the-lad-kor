...
OUT = ROOT / "03_output"
STEM = "arc1_v292_bdf_pieces_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v292_bdf_pieces"
...
def main() -> None:
    ...
    # PSX.EXE strings are left out on purpose.  The filter rejects executable
    # code but not data tables, and writing glyphs into those killed the boot.
    # This build isolates the variable: only DAT dialogue is touched.
    step("EXE 문자열 제외 - 대사만 재인코딩")
    ...
    step("조각 렌더")
    # BDF-extracted pieces, not a TTF render: rendering the TTF at 16px shifts
    # glyphs inside the cell so composed syllables come out mangled.
    _raw = (ROOT / "01_work/analysis/hangul_johab_16px/pieces_1bpp.bin").read_bytes()
    ...
    pieces = [[struct.unpack_from(">H", _raw, (i * 16 + y) * 2)[0] for y in range(16)]
              for i in range(360)]
    ...
    slot = {}
    kpic = {}
    # bible_current.txt: verified-safe one-byte codes are 0x68..0xD0 plus D4/D8/DC.
    # 0x08..0x64 are Japanese glyph slots shared with menus, numbers, HP/MP and
    # name windows -- writing Korean there bleeds into UI and untranslated text.
    SAFE = [c - 1 for c in list(range(0x68, 0xD1)) + [0xD4, 0xD8, 0xDC]]
    one_taken = set(a1.values())
    free_one = [i for i in SAFE if i not in one_taken]
    if not free_one:
    ...
    rep = [
        "v292 TEST ONLY - 16px from BDF pieces, safe slot range",
