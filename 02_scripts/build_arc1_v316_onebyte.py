"""Build v316: jung and jong take the verified safe one-byte codes.
...
OUT = ROOT / "03_output"
STEM = "arc1_v316_onebyte_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v316_onebyte"
...
# Second slot bank.  Per-line placement needs about three times the slots that
# whole-body placement did, and a scene file's nominal bank stops at 0x477FF
# because dialogue starts near 0x47800.  File 0x004200 is the largest run that
# is zero in all 160 files that hold translated dialogue (28 blocks of 128B).
# No extra arithmetic is needed: the address is 0x80114000 + slot * 0x80, so a
# negative slot number reaches backwards.  0x004200 is slot -2076.
BANK_B_FILE, BANK_B_SLOTS = 0x004200, 28
BANK_B_FIRST = SLOT_MAX                       # logical slot number of bank B[0]

CAVE_START, CAVE_END = 0x8018FCD0, 0x8018FDC5
# The advance helper moves out of the shared cave so the two E2 handlers can
# grow a bank-B branch.  0x80193B44 is 128 zero bytes in the original image and
# is untouched in the user's save states (0x801A7xxx is not - the game writes
# there, exactly as codex_notes warned).
HELPER_CAVE, HELPER_CAVE_END = 0x80193B44, 0x80193BC4
LOOKUP_HANDLER, COMPLETION_HANDLER = 0x8018FCD0, 0x8018FD38
ADVANCE_HELPER = HELPER_CAVE
...
# Dialogue window width in glyph cells.  Measured from a screenshot: the line
# `약속대로 이 마을에서 내보내` is 14 cells and the next glyph wrapped, so 19
# (the earlier guess) pushed one syllable onto a line of its own every time.
WINDOW_COLS = 14

# bible_current.txt records disk codes 0x68..0xD0 plus D4/D8/DC as the range
# whose original glyphs are safe to overwrite -- proven by story_test_11 on
# hardware.  As indices that is 0x67..0xCF.  Putting jung and jong there makes
# them one byte instead of two, which cuts a syllable from 6 bytes to 4 and the
# speaker name 촌장 from 12 to 8.  Choseong cannot use it: the advance test has
# to tell pieces from ordinary glyphs, and ASCII already lives below 103.
SAFE1_LO, SAFE1_COUNT = 0x67, 105
...
def lookup_handler():
    """E2 argument (already nn-1) -> slot address in v0, or tail-call the original.

        andi  t0,a0,0xFF
        sltiu t1,t0,0x80
        bne   t1,zero,orig          ; below 0x80 is a name lookup
         addiu t2,t0,-0xA8
        beq   t2,zero,orig          ; disk A9 is the original's own id
         sltiu t1,t0,0xA8
        bne   t1,zero,lo            ; 0x80..0xA7  -> slot 0..39
         sltiu t1,t0,0xD0
        bne   t1,zero,hi            ; 0xA9..0xCF  -> slot 40..78
         sltiu t1,t0,0xEC
        beq   t1,zero,orig          ; 0xEC and up is not ours
         addiu t0,t0,-2284          ; 0xD0..0xEB -> bank B, negative slot
        b     common
         nop
    lo: addiu t0,t0,-0x80
        b     common
    ...
    """
    b = LOOKUP_HANDLER
    lo, hi, common, orig = b + 0x38, b + 0x44, b + 0x48, b + 0x60
    w = [0x308800FF, 0x2D090080,
         branch(0x05, 9, 0, b + 0x08, orig), 0x250AFF58,
         branch(0x04, 10, 0, b + 0x10, orig), 0x2D0900A8,
         branch(0x05, 9, 0, b + 0x18, lo), 0x2D0900D0,
         branch(0x05, 9, 0, b + 0x20, hi), 0x2D0900EC,
         branch(0x04, 9, 0, b + 0x28, orig), 0x2508F714,
         branch(0x04, 0, 0, b + 0x30, common), 0x00000000,
         0x2508FF80,
         branch(0x04, 0, 0, b + 0x3C, common), 0x00000000,
         0x2508FF7F,
         0x000811C0, 0x3C098011, 0x25294000, 0x00491021,
         0x03E00008, 0x00000000,
         jj(LOOKUP), 0x00000000]
    assert len(w) == 26, len(w)
    return struct.pack("<%dI" % len(w), *w)


def completion_handler():
    """After the secondary string ends, skip the inline bytes it replaced.

    Same id ranges as the lookup handler, but this one reads the disk byte
    itself (nn, not nn-1), so every boundary constant is one higher.
    """
    b = COMPLETION_HANDLER
    lo, hi, common, done = b + 0x44, b + 0x50, b + 0x54, b + 0x74
    w = [0x8E080014, 0x00000000, 0x9109FFFF, 0x00000000,
         0x2D2A0081,
         branch(0x05, 10, 0, b + 0x14, done), 0x252BFF57,
         branch(0x04, 11, 0, b + 0x1C, done), 0x2D2A00A9,
         branch(0x05, 10, 0, b + 0x24, lo), 0x2D2A00D1,
         branch(0x05, 10, 0, b + 0x2C, hi), 0x2D2A00ED,
         branch(0x04, 10, 0, b + 0x34, done), 0x2529F713,
         branch(0x04, 0, 0, b + 0x3C, common), 0x00000000,
         0x2529FF7F,
         branch(0x04, 0, 0, b + 0x48, common), 0x00000000,
         0x2529FF7E,
         0x000949C0, 0x3C0A8011, 0x254A4000, 0x012A4821,
         0x912A007F, 0x00000000, 0x010A4021, 0xAE080014,
         0x34020001, jj(COMPLETION_TARGET), 0x00000000]
    assert len(w) == 32, len(w)
    return struct.pack("<%dI" % len(w), *w)
...
def advance_helper(bound):
    ...
    label = base + 10 * 4
    lo, count = bound
    if not 0 <= count <= 0x7FFF:
        raise ValueError("count out of range")
    w = [0x90C5000F,                       # lbu   a1,0xF(a2)
         0x2401_0000 | ((-lo) & 0xFFFF),   # addiu at,zero,-lo   (placeholder)
         0x00810821,                       # addu  at,a0,at      at = index - lo
         0x2C210000 | count,               # sltiu at,at,count   1 = piece
         0x94C4000A,                       # lhu   a0,0xA(a2)
         0x94C20006,                       # lhu   v0,6(a2)
         branch(0x05, 1, 0, base + 6 * 4, label),   # bne at,zero,L  (piece: no advance)
         0x00651821,                       # addu  v1,v1,a1
         0x00431021,                       # addu  v0,v0,v1
         0xA4C20006,                       # sh    v0,6(a2)
         0x24840001,                       # L: addiu a0,a0,1
...
def main():
    ...
    # The safe one-byte range is deliberately overwritten, so it is not part of
    # the untouched-glyph guarantee.  Everything else must stay byte-identical.
    inked = [i for i in reach if ink(i) is True
             and not (SAFE1_LO <= i < SAFE1_LO + SAFE1_COUNT)]
    ...
    if len(need_rest) > SAFE1_COUNT:
        raise SystemExit("jung/jong %d exceed the safe one-byte range %d"
                         % (len(need_rest), SAFE1_COUNT))
    rest_cells = list(range(SAFE1_LO, SAFE1_LO + len(need_rest)))
    bound = (SAFE1_LO, len(need_rest))
    below = list(free)
    ...
    step("조각 구간 index %d..%d (%d칸, 1바이트 코드)  초성/기호용 빈 칸 %d개"
         % (SAFE1_LO, SAFE1_LO + len(need_rest) - 1, len(need_rest), len(below)))
    ...
    def slot_offset(s):
        if s < SLOT_MAX:
            return SLOT_BASE + s * SLOT_SIZE
        return BANK_B_FILE + (s - BANK_B_FIRST) * SLOT_SIZE

    def disk_id(slot):
        if slot < 40:
            return slot + 0x81                       # bank A, ids 0x81..0xA8
        if slot < SLOT_MAX:
            return slot + 0x82                       # bank A, ids 0xAA..0xD0
        return slot - BANK_B_FIRST + 0xD1            # bank B, ids 0xD1..0xEC
    ...
    for n in set(l[0] for l in lines):
        d = mem[n]
        fs = []
        for s in list(range(SLOT_MAX)) + list(range(BANK_B_FIRST, BANK_B_FIRST + BANK_B_SLOTS)):
            at = slot_offset(s)
            if at + SLOT_SIZE <= len(d) and not any(d[at:at + SLOT_SIZE]):
                ...
                if s >= BANK_B_FIRST:
                    bank_b_total += 1
        free_slots[n] = fs
    step("빈 슬롯 %d개 (뱅크 A %d + 뱅크 B %d)"
         % (sum(len(v) for v in free_slots.values()),
            sum(len(v) for v in free_slots.values()) - bank_b_total, bank_b_total))

    scratch = {n: bytearray(v) for n, v in mem.items()}
    inline = moved = trunc = no_slot = ctrl_kept = 0
    multi = few_slots = short_head = 0
    def fold(t, width=WINDOW_COLS):
        out, cur = [], ""
        for word in t.split(" "):
            if not cur:
                cur = word
    ...
    def wrap_to(t, k):
        """Split the translation across the k lines the original body provides.

        The renderer wraps a secondary string on its own, so a body given as one
        E2 flows past the window and the tail is lost -- that is what cut
        `해 줄 거지?` down to `해 구`.  Feeding one line per original span keeps
        the layout the game already designed for the scene.
        """
        out = fold(t)
        if len(out) > k:
            out = (out[:k - 1] + [" ".join(out[k - 1:])]) if k > 1 else [" ".join(out)]
        while len(out) < k:
            out.append("")
        return out

    def split_body(txt, spans, body):
        """The original keeps the speaker on a line of its own:

            村長 \\n さあ、この先から先は \\n おまえ達ワイト家の者しか入れない。

        Our CSV stores that as one string, `촌장: 자, 이 앞부터는 ...`.  Folding
        the whole thing across the spans pushed dialogue into the name line and
        the `촌장 :` form disappeared from the screen.  So peel the name off and
        give it the span the original gave it.
        """
        k = len(spans)
        if k >= 2:
            head_len = spans[0][1] - spans[0][0]
            name, sep, rest = txt.partition(":")
            if sep and rest.strip() and len(name) <= 8 and head_len <= 12:
                return [name.strip()] + wrap_to(rest.strip(), k - 1)
        return wrap_to(txt, k)
    ...
    for n, off in lines:
        ...
        while i < len(body):
            ...
            if b >= 0xE1:
                ...
                continue
            j2 = i
            while j2 < len(body) and body[j2] < 0xE1:
                j2 += 1 if body[j2] < 0xDD else 2
            spans.append((i, min(j2, len(body))))
            i = j2
        if not spans:
            continue
        buf = scratch[n]
        ...
        if fs is None:
            fs = free_slots[n] = []
        parts = split_body(txt, spans, body)
        if len(spans) > 1:
            multi += 1
        for (a, b), part in zip(spans, parts):
            ...
            if len(code) <= room:
                ...
                inline += 1
                continue
            if room >= 2 and fs and len(code) <= SLOT_SIZE - 2:
                s = fs.pop(0)
                at = slot_offset(s)
                buf[at:at + len(code)] = code
                buf[at + len(code)] = 0
                # skip the rest of this span so parsing resumes at its control code
                buf[at + 0x7F] = (room - 2) & 0xFF
                buf[off + a] = 0xE2
                buf[off + a + 1] = disk_id(s)
                moved += 1
                continue
            ...
            if not fs:
                no_slot += 1
            elif len(code) > SLOT_SIZE - 2:
                few_slots += 1
            else:
                short_head += 1
    step("줄 단위 배치: 인라인 %d줄  슬롯 %d줄  잘림 %d줄 "
         "(슬롯 없음 %d, 한 슬롯 초과 %d, 자리 부족 %d)  여러 줄 대사 %d개"
         % (inline, moved, trunc, no_slot, few_slots, short_head, multi))
    ...
    helper_cave = fo(HELPER_CAVE)
    if any(exe[helper_cave:fo(HELPER_CAVE_END)]):
        raise SystemExit("GUARD: helper cave is not zero")
    lk, cp, ah = lookup_handler(), completion_handler(), advance_helper(bound)
    for addr, blob, end in ((LOOKUP_HANDLER, lk, CAVE_END),
                            (COMPLETION_HANDLER, cp, CAVE_END),
                            (ADVANCE_HELPER, ah, HELPER_CAVE_END)):
        if addr + len(blob) > end:
            raise SystemExit("GUARD: %08X overflows its cave" % addr)
        exe[fo(addr):fo(addr) + len(blob)] = blob
    if LOOKUP_HANDLER + len(lk) > COMPLETION_HANDLER:
        raise SystemExit("GUARD: the two handlers overlap")
    ...
    step("핸들러 심음. E2 cave %d/%dB, helper cave %d/%dB"
         % (len(lk) + len(cp), CAVE_END - CAVE_START,
            len(ah), HELPER_CAVE_END - HELPER_CAVE))
    ...
    rep = [
        "v316 TEST ONLY - one-byte jung/jong",
        "output=%s" % out.name,
        "sha256=%s" % st,
        ...
        "pieces occupy index %d..%d and draw without moving the cursor"
        % (SAFE1_LO, SAFE1_LO + len(need_rest) - 1),
        "placement is per original line: one E2 per text span, controls stay put",
        "lines %d  translated %d  inline lines %d  slot lines %d  cut lines %d"
        % (len(lines), len(trans), inline, moved, trunc),
        "cut causes: no free slot %d, over one slot %d, span too short for E2 %d"
