"""v130: put the current translation into the game, with the two encoder faults fixed.

Three separate things were wrong in v129 and this build addresses all three.

  the vertical smear   The font occupies two texture pages vertically.  V is right for
                       both halves; the page is chosen by the classifier, which knows
                       only the four strip V values.  25 characters were being encoded
                       to ordinary font rows 31..39, on the second page, and drew twelve
                       rows of some other cell.  `drawable()` in the planner now refuses
                       those indices, so the encoder hands out the strip A twin instead.

  the freeze           A slot is 128 bytes: text, a 0x00 terminator, then the completion
                       metadata in byte 0x7F.  `SLOT_TEXT_MAX` was 127, which leaves no
                       room for the terminator and the metadata write lands on byte 127
                       regardless, so the string never ends.  Seven slots shipped that
                       way.  The limit is 126 now, and this build additionally refuses to
                       finish if any slot it wrote lacks a terminator.

  the missing edits    The externally built v126 did not apply the edited CSV; 1,468
                       Korean lines in `script_translated_full.csv` are not what the disc
                       holds.  This build writes them.

It works on v129 in place rather than restacking from the original disc, because v128's
battle choices live in E5 bodies that the planner deliberately never touches, and
restacking would silently revert them.  Only bodies this build can prove out get
rewritten; everything else keeps the bytes v129 had.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v129_remap_coverage_430F1CFD.zip"
BASE_SHA = "430F1CFDF91E6C58AEF937C5FBD5099B4CEAF745783EBCD207DF0337AE72B289"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v130_text_reinsert"
ANALYSIS = ROOT / "01_work/analysis/arc1_v130_text_reinsert"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v129")
    ...
    lines = [
        "v130 the current translation, with the two encoder faults fixed",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(rebuilt)}; PSX.EXE and COMM.IMG byte-identical to v129",
        "",
