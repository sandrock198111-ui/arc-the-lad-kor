...
OUT = ROOT / "03_output/arc1_v206_restore_zeroed_script_data.zip"
...
def main() -> None:
    ...
    for name, current in before.items():
        ...
        if len(original) != len(current):
            continue
        # a zero whose neighbour is non-zero is a string terminator we wrote.
        # restoring it would splice the next run onto the end of our line --
        # that is exactly how 32/S3051 slot 1 grew from 31 to 73 bytes.
        holes = [i for i in range(len(original))
                 if original[i] and not current[i]
                 and not (i and current[i - 1])]
        if not holes:
            continue
    ...
    for name in restored:
        ...
        for i in range(len(made)):
            ...
            elif made[i] and made[i] != original[i]:
    ...
    print("v206  옛 재삽입이 0으로 지운 스크립트 바이트 복구 (종료자 제외)")
