"""v136: the edits made since v134, put in.

The 14 lines whose Korean had been reduced to question marks in the CSV -- a whole
scene, the Smeria king's death -- are translated again; one of them, `22/S2052.DAT
0x47BEA`, still had its Korean on the disc and was simply copied back. Three drafts had
to avoid 김, 높 and 깃, which have no glyph anywhere.

Also the hand edits to `22/S2053.DAT`: 아크's line now says 아시겠지요 so that the
minister's 아시겠지요!? has something to throw back, which is what the Japanese does
with わかった / 「わかつたか」だと.

Same in-place pass as v133 and v134, same guards. Choice bodies stay as v135 left them.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v135_choices_in_place_DE3A5A08.zip"
BASE_SHA = "DE3A5A08C5D735122264E573B48859113593C0B6B2AB9EB00599C7484AA6187A"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v136_scene_restore"
ANALYSIS = ROOT / "01_work/analysis/arc1_v136_scene_restore"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v135")
    ...
    lines = [
        "v136 the scene that was lost to question marks, and the day's edits",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(rebuilt)}; PSX.EXE and COMM.IMG byte-identical to v135",
        "",
