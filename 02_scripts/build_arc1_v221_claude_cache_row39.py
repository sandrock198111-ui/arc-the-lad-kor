#!/usr/bin/env python3
"""v221 CLAUDE: park the dynamic cache on the one row the game never touches.

v211 moved the 28-slot cache off the world-map art and onto virtual row 32.  It
picked that row without measuring, and row 32 turns out to be one of the busier
ones.  v212 through v219 then answered the leftover collisions by choosing
between two destinations every frame -- a selector that has not booted cleanly
yet.

The collision does not need a selector.  It needs a row nobody else wants.

Measured over the 280 savestates in vram_occupancy_map/page15_rows.txt, and
independently over the 9 raw VRAM dumps in vram_survey_2026-08-02:

    row 37  y444..455    23 states     game art
    row 38  y456..467     0 states     free in every sample
    row 39  y468..479     0 states     free in every sample
    row 40  y480..491    98 states     where the cache sits today
    row 41  y492..503    73 states     game art

Rows 38 and 39 are the only twelve-line bands inside page 15,1 that no state
touches, over the whole 64-halfword width -- a stricter test than the 15
halfwords the cache actually occupies.  The raw dumps agree: 0 non-zero
halfwords, so neither resident strip lives there either.

This takes row 39, immediately above the current position.  Everything else --
X, U, the 28 slots, the sources, the lookup table -- is untouched, and the edit
is the same four instructions and one rectangle v211 established.
"""
...
OUT_STEM = "arc1_v221_CLAUDE_cache_row39_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v221_CLAUDE_cache_row39"
REPORT = ANALYSIS / "build_report.txt"

PSX = "PSX.EXE"
OLD_ROW, NEW_ROW = 40, 39
OLD_Y, NEW_Y = 480, 468
OLD_V, NEW_V = 224, 212
...
def main() -> None:
    ...
    report = [
        "v221 CLAUDE TEST ONLY - cache parked on measured-free row 39",
        f"base={BASE.name}",
        f"base_sha256={BASE_SHA256}",
        f"output={output.name}",
        f"sha256={stamp}",
        "release_status=DIAGNOSTIC; DO NOT DISTRIBUTE",
        "row39_states_touching=0/280", "row39_raw_vram_nonzero=0/9", "no_AB_selector=collision removed by placement",
        ...
        "expected=world map clean AND cache glyphs intact; no per-frame selector involved",
        "known_risk=none measured; row39 free in 280/280 states and 9/9 raw VRAM dumps",
