"""Build v309: 12px johab from the original disc, Sans pieces.
...
The piece set keeps the original 8-beol choseong and 4-beol jungseong and only
folds jongseong 4 -> 2.  Encoding order is jung -> jong -> cho so the choseong
is always last: the renderer advances the cursor only for codes below BOUND, so
jung/jong draw in place and cho closes the syllable.
...
OUT = ROOT / "03_output"
STEM = "arc1_v309_johab12_sans_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v309_johab12_sans"
...
def main():
    ...
    piece = [render(chr(0xF600 + i)) for i in range(360)]
    ...
    def decompose(ch):
        ...
        vb = (1 if cho in (0, 15) else 0) + (2 if jong else 0)
        tb = JONG_FOLD[grp(jung)]
        return ("c", cb, cho), ("v", vb, jung), (("t", tb, jong) if jong else None)
    ...
    rep = [
        "v309 TEST ONLY - 12px johab from the original disc, Sans pieces",
        "output=%s" % out.name,
        "sha256=%s" % st,
        "release_status=DIAGNOSTIC; DO NOT DISTRIBUTE",
        "font=%s at %dpx, cho 8beol + jung 4beol + jong 2beol" % (FONT_NAME, CELL),
