...
STEM = "arc1_v254_body_only_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v254_body_only"
...
def main() -> None:
    ...
    for r in rows:
        ...
        if cap >= 2 and buf[off] == 0xE2:
            # E2 xx points at text held elsewhere.  Only 79 of the 1445 such
            # references resolve through the slot table, and writing the other
            # 1366 in place destroys the pointer itself -- which is what hung
            # v251/v253.  Leave every E2 line exactly as it was.
            skipped_ref += 1
            continue
    ...
    report = [
        "v254 TEST ONLY - body lines only; E2 reference lines untouched",
        ...
        f"lines written={wrote}   truncated={truncated}   E2 refs left alone={skipped_ref}",
