...
OUT = ROOT / "03_output"
STEM = "arc1_v302_ui_exact_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v302_ui_exact"
...
def main() -> None:
    ...
    for at in range(0, len(exe) - 4, 4):
        ...
        if 0 <= a < len(exe):
            ptr_targets.add(a)
    ui = {o: v for o, v in ui.items() if o in ptr_targets}
    step(f"포인터가 실제로 가리키는 UI 문자열만 남김: {len(ui)}곳")
    ...
    tmp.replace(out)
    rep = [
        "v302 TEST ONLY - UI written only at real pointer targets",
