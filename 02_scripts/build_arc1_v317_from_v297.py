"""Build v317: v297 exactly, with the measured beol rule and Pilgi pieces.
...
OUT = ROOT / "03_output"
STEM = "arc1_v317_from_v297_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v317_from_v297"
...
def main() -> None:
    ...
    # Beol rule derived by matching every piece combination against this font's
    # own completed syllables (646 of them).  The textbook "group + 4 when a jong
    # is present" is wrong here: beol 4 is never selected, B and C share beol 6
    # once a jong appears, the jungseong ignores a g/k choseong, and every
    # jongseong uses beol 1.  Guessing it put the RIEUL of 롭 in the corner.
    ...
    def compose(ch):
        if "가" <= ch <= "힣":
            ...
            p = [pieces[cb * 20 + cho + 1],
                 pieces[160 + (2 if jong else 0) * 22 + jung + 1]]
            if jong:
                p.append(pieces[248 + 1 * 28 + jong])
    ...
    tmp.replace(out)
    rep = [
        "v317 TEST ONLY - v297 base, measured beol rule, Pilgi",
