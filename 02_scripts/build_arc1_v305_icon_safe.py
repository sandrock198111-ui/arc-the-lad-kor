...
OUT = ROOT / "03_output"
STEM = "arc1_v305_icon_safe_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v305_icon_safe"
...
def main() -> None:
    ...
    # Icons are sampled by UV, not by glyph code, so "no dialogue references it"
    # never catches them -- that is why v298-v303 broke the battle icons.
    # The only trustworthy list is what the shipped v197 actually wrote, plus
    # cells the original leaves completely blank.
    safe = pickle.load(open(ROOT / "01_work/analysis/safe_cells_v197.pkl", "rb"))
    avail = [i for i in safe if i not in keep and not hits_cursor(i)]
    step(f"v197 검증 + 원본 공백 기준 안전 칸 {len(avail)}개")
    ...
    ui_ok = ui_skip = 0
    for off, (L, t) in sorted(ui.items()):
        code = bytearray()
        for ch in t:
            c = sp if ch == " " else (encode(slot_ix[ch]) if ch in slot_ix else None)
            if c:
                code += c
        if not code or len(code) >= L:
            ui_skip += 1
            continue
        exe[off:off + len(code)] = code
        exe[off + len(code)] = 0
        for k in range(off + len(code) + 1, off + L):
            exe[k] = 0
        ui_ok += 1
    step(f"UI 적용 {ui_ok}곳,  자리 부족으로 보류 {ui_skip}곳")
    cave = fo(CAVE_START)
    ...
    tmp.replace(out)
    rep = [
        "v305 TEST ONLY - glyph cells restricted to v197-verified safe list",
