...
OUT = ROOT / "03_output"
STEM = "arc1_v300_ui_repack_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v300_ui_repack"
...
def main() -> None:
    ...
    for at in range(0, len(exe) - 4, 4):
        ...
        if BLK_LO <= a < BLK_HI:
            ptr_of[a].append(at)
    order = sorted(ptr_of)
    step(f"UI 블록 문자열 {len(order)}곳,  포인터 {sum(len(v) for v in ptr_of.values())}개")
    ...
    tmp.replace(out)
    rep = [
        "v300 TEST ONLY - UI block repacked with pointer rewrite (v197 method)",
