#!/usr/bin/env python3
"""v224 CLAUDE: v222 with the cache-detect constant left alone.

v222 put the cache on the rectangle the game never overwrites and the scenery
bleed stopped -- the first build where it did.  What stayed broken was blank
cells, and reading the drawn packets out of the savestates says why:

    v222   V=164 correct (24..96 packets drawn)   U base 0, expected 156
    v223   V=224 wrong                            U base 4

V comes from the cache index base, so v222 had it right.  U comes from the low
helper at 0x801A2060:

    addiu a3, t0, -0x28     test t0 == 40, i.e. is this a cache glyph
    bnez  a3, skip
    lbu   a3, 0x28(a1)      read the packet U
    addiu a3, a3, 4         add the cache X offset
    sb    a3, 0x28(a1)

The 0x28 in the first line is a detect constant; the 4 in the fourth is the X
offset.  v222 rewrote both because v211 treated them as one number.  With the
test moved to 35 the whole block stopped running, so U never got its offset and
the packets read x960 while the upload wrote x999.

So this keeps every v222 field except that test, which goes back to 40.

    cache index base 35*84   ->  V 164      (as v222)
    low helper test  40      ->  unchanged  (v222 had 35)
    packet U         4 -> 156, X 961 -> 999, y 480 -> 420
"""
...
OUT_STEM = "arc1_v224_CLAUDE_keep_cache_probe_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v224_CLAUDE_keep_cache_probe"
...
def main() -> None:
    ...
    report = [
        "v224 CLAUDE TEST ONLY - v222 placement with the cache-detect constant kept at 40",
        ...
        "expected=no scenery bleed (as v222) AND packet U base 156 so glyphs render",
    ]
