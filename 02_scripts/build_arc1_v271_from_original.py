...
OUT = ROOT / "03_output"
STEM = "arc1_v271_from_original_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v271_from_original"
SCENE = "1/S1011.DAT"
...
def main() -> None:
    ...
    for n, d in before.items():
        if not n.upper().endswith(".DAT") or len(d) <= BASE_OFF:
            continue
        s = scan(d)
        if s:
            sites[n] = s
    ...
    rep = [
        "v271 TEST ONLY - rebuilt from the original disc, 16px, opening scene in Korean",
