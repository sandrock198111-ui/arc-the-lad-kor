"""v142: the choice bodies again, on top of the tightened script.

Same model as v137 and v140: every E5 and E6 stays at its original byte offset, and a
span too small for its Korean takes a two-byte E2 with the text in a slot. Rerun because
v141 freed slots the choice redirects can now use.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v141_tightened_3ACBA9A1.zip"
BASE_SHA = "3ACBA9A16A7300E1C96E61E9433728C071F794EF862F7CA295EA21864094D715"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v142_choices_final"
ANALYSIS = ROOT / "01_work/analysis/arc1_v142_choices_final"
LINEBREAK = b"\xE6\x01"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v141")
    ...
    lines = [
        "v142 choice bodies, final pass",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(members)}; PSX.EXE and COMM.IMG byte-identical to v141",
        "",
