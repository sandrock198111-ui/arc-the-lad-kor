#!/usr/bin/env python3
"""v222 CLAUDE: move the glyph cache to the only rectangle the game never touches.

The cache has been sitting at x961..981, y480..491.  Folding all 467 savestates
on this machine into one VRAM occupancy map, that rectangle is used by the game
in every single one of its 252 halfwords.  It was never a free spot; the game
loads texture there, which is why translated glyphs turned into scenery after a
world map.  The damage runs game -> us, not us -> game.

v211 tried row 32, and v212..v219 answered the leftover collision with a
per-frame A/B selector.  Neither addressed the cause: at x961 there is no free
row to move to.

Searching the whole 467-state union for a free 21x12 rectangle -- 21 halfwords
because the cache is 7 cells wide, not the 5 earlier notes assumed -- leaves
exactly two, and both start right of x975.  Intersected with the row grid, one
position survives:

    row 35   y420..431   x999..1019     0 of 252 halfwords ever used
    row 40   y480..491   x961..981    252 of 252 halfwords used  (today)

So this moves the cache horizontally as well as vertically, which v211 never
did.  Eight fields change: the virtual row, the cache index base, the two V
biases, the two U biases, the X bias, and the upload rectangle.

    row 40 -> 35     y 480 -> 420     V 224 -> 164
    x 961 -> 999     U   4 -> 156

U 156 + 84 pixels = 240 and V 164 + 12 = 176, so both stay inside the 256-unit
texture page, and x999..1019 stays inside page 15,1 -- the tpage word is
unchanged.
"""
...
OUT_STEM = "arc1_v222_CLAUDE_cache_row35_x999_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v222_CLAUDE_cache_row35_x999"
...
def main() -> None:
    ...
    I = old.i_type
    edits = [("virtual row", low_row_at,
              I(0x09, v171.T0, v171.A3, -OLD_ROW),
              I(0x09, v171.T0, v171.A3, -NEW_ROW))]
    ...
    report = [
        "v222 CLAUDE TEST ONLY - cache moved to the only never-used 21x12 rectangle",
        ...
        "expected=world map clean AND cache glyphs survive scene changes",
    ]
