"""v141: the script again, after 100 lines were tightened by a byte or two.

Five files had more lines needing an external slot than they had slots, and the fix was
not to shorten all 212 of them: a line that fits where the Japanese sat needs no slot at
all, so freeing one frees it for another. 186 lines had to move, 102 of them short by
one or two bytes, and closing a space in those is invisible -- 99 were done that way and
one lost a full stop. Exclamation and question marks were left alone; they carry tone.
"""
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v141_tightened"
ANALYSIS = ROOT / "01_work/analysis/arc1_v141_tightened"
...
def main() -> None:
    ...
    lines = [
        "v141 the script after tightening",
        "",
