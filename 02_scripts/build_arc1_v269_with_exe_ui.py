...
STEM = "arc1_v269_with_exe_ui_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v269_with_exe_ui"
...
def sites(exe: bytes) -> dict[str, list[tuple[int, int]]]:
    """Dialogue slots from the DAT files, plus the UI strings inside PSX.EXE.

    v268 shipped without the second half and the load screen came out as a grid
    of garbage: those strings kept their old code numbers while the atlas was
    ...
    start = EXE_POOL[0]
    for cur in range(EXE_POOL[0], EXE_POOL[1]):
        if exe[cur]:
            continue
        if cur > start:
            out[PSX].append((start, cur - start))
        start = cur + 1
    return out
...
def main() -> None:
    ...
    # the EXE re-encode happened in `scratch`; fold it into the patched exe
    for off, ln in site_map.get(PSX, []):
        exe[off:off + ln] = scratch[PSX][off:off + ln]
    ...
    tot = sum(uses.values())
    rep = [
        "v269 TEST ONLY - full site list plus the UI strings in PSX.EXE",
