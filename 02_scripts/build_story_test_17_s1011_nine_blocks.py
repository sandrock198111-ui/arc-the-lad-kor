...
WORK = ROOT / "01_work" / "story_test_17_s1011_nine_blocks"
WORK_FONT = WORK / "COMM.IMG"
WORK_S1071 = WORK / "1" / "S1071.DAT"
WORK_S1011 = WORK / "1" / "S1011.DAT"
OUTPUT = ROOT / "03_output" / "story_test_17_s1011_nine_blocks_patch_only.zip"
...
def write_text_files() -> None:
    test_info = """Arc the Lad 1 JP Korean Patch - Story Test 17 S1011 nine blocks

Purpose:
- Stop single-block stepping and validate a file-range batch for S1011.
- Keep COMM.IMG and S1071.DAT unchanged from story_test_16.

Change:
- Keep S1011 blocks 1-4 from story_test_16.
- Add S1011 blocks 5-9:
  5. 아크 / 조심하라 / 기다리겠다
  6. 가라
  7. 마을로 / 돌아가라
  8. 촌장 / 아크여 / 돌아올 때까지
...
- Confirm the first, middle, and last changed S1011 blocks.
- Confirm no freeze after the last changed block.
- Confirm no title/logo corruption.
"""
    charmap = """Story test 17 S1011 nine-block map

Same character map as story_test_16.
...
def main() -> None:
    ...
    if WORK_FONT.exists() or WORK_S1071.exists() or WORK_S1011.exists() or OUTPUT.exists():
        raise SystemExit("Story test 17 already exists; refusing to overwrite it.")
    ...
    patches = [
        ...
        (0x47B40, 47, 0x47B6F, ["촌장", "아크여", "돌아올 때까지"]),
        (0x47BB2, 5, 0x47BB7, ["예"]),
    ]
