#!/usr/bin/env python3
"""Independent static verifier for the V357 user-reviewed Bank-B build.

This verifier deliberately does not import the V357 builder or trust its
...
FULL = ROOT / "03_output/arc1_v357_user_reviewed_full_dialogue_bankb_TEST_ONLY_34854022.zip"
FULL_SHA256 = "34854022BB0CA1C17BF4D798B8405009F8CC04914253E1D48476B2A2DBC3397A"
DELTA = ROOT / "03_output/arc1_v357_user_reviewed_full_dialogue_bankb_TEST_ONLY_delta_from_v354_5A285CFD.zip"
DELTA_SHA256 = "5A285CFDCFD686E114B2B3D814C6FB1079FF0B49DAFE2A8EB7C3604BF2152783"
...
EXPECTED = ROOT / "01_work/analysis/arc1_v357_user_reviewed_full_dialogue_bankb/expected_writes.csv"
MANIFEST = ROOT / "01_work/analysis/arc1_v357_user_reviewed_full_dialogue_bankb/build_manifest.json"
...
def main() -> None:
    ...
    if len(changed) != 73 or delta_names != changed:
    ...
    if final[PSX][HANDLER_FILE:HANDLER_FILE + HANDLER_SIZE] != v355_exe[HANDLER_FILE:HANDLER_FILE + HANDLER_SIZE]:
        raise VerifyError("V357 handler differs from runtime-approved V355")
    ...
    if len(targets) != 353 or len({(r["source file"], r["offset"]) for r in targets}) != 353:
    ...
    if Counter(row["classification"] for row in targets) != {
        "빌드대기": 172, CHOICE: 134, REVIEW_NEEDED: 47,
    }:
        raise VerifyError("target classification differs")
    approved_review_keys = {
        (row["source file"], row["offset"]) for row in targets
        if row["classification"] == REVIEW_NEEDED
        and row["review_status"] == "user_approved_export_20260903"
    }
    if len(approved_review_keys) != 47:
        raise VerifyError("47-row user-review approval boundary differs")
    ...
    if len({row["source_file"] for row in visible_audit}) != 72:
    ...
    if manifest["target_rows"] != 353 or manifest["already_current_rows"] != 5:
        raise VerifyError("manifest target census differs")
    if (
        manifest["review_pending"] != 0
        or manifest["user_reviewed_bank_b_rows"] != 47
        or manifest["additional_vram_bytes"] != 0
    ):
    ...
    result = {
        "verdict": "STATIC PASS / USER-REVIEWED / RUNTIME PENDING / TEST_ONLY",
    ...
    (ANALYSIS / "independent_verification.txt").write_text(
        "\n".join([
            "Arc the Lad 1 V357 independent static verification",
            result["verdict"],
            ...
            f"changed members: {result['changed_members']}/73",
            f"Expected-Write bytes: {result['expected_writes']}",
            "COMM.IMG: byte-identical; added VRAM: 0",
            "user review: 47 former Bank-B draft rows approved in the 2026-09-03 export",
            "runtime: PENDING cold boot and representative traversal",
            "",
        ]),
        encoding="utf-8",
    )
    print("V357 independent verification PASS")
