#!/usr/bin/env python3
"""Build V357: reinsert the user-reviewed full dialogue with Bank-B support.
...
The 2026-09-03 editor export was explicitly accepted by the user as the prose
source, with the requested terminology exceptions and duplicate speaker prefix
repair. Runtime traversal is still required, so this remains TEST_ONLY.
"""
...
VERSION = "V357"
OUTPUT_STEM = "arc1_v357_user_reviewed_full_dialogue_bankb_TEST_ONLY"
DELTA_STEM = OUTPUT_STEM + "_delta_from_v354"
ANALYSIS = ROOT / "01_work/analysis/arc1_v357_user_reviewed_full_dialogue_bankb"
OUT = ROOT / "03_output"
...
CANONICAL_SHA256 = "F751A145C08423067CED1D5AF288590E3F1E2E38514103D43EF531681FA2E274"
...
TARGETS = ROOT / "05_docs/v357_full_dialogue_targets.csv"
TARGETS_SHA256 = "0DC7ADA11DE5F1A99D64BC5B845E802F38AA7CC1F1DA14873D05BA3C476462F8"
...
CONTROL_TEMPLATES: dict[tuple[str, int], str] = {
    ("21/S2041.DAT", 0x483D4):
        "승무원:|아니요,{E4:1F}{E4:1F}{E4:1F}아무것도 아닙니다.",
    ...
    ("32/S3061.DAT", 0x47BB4):
        "은혜의 정령:|그래요. 스메리아의 황태자였던|요슈아 왕자,{E4:29}|그분이 당신의 아버지예요.",
    ("4/S4021.DAT", 0x47AFA):
...
def main() -> None:
    ...
    if (len(canonical), len(original), len(target_rows), len(nontext_rows)) != (
        2878, 2878, 353, 199,
    ):
    ...
    if Counter(
        row["review_status"] for row in target_rows
        if row["classification"] == "B검수필요"
    ) != {"user_approved_export_20260903": 47}:
        raise BuildError("47 Bank-B rows are not bound to the user-approved export")
    ...
    for member in changed_members:
        reason = "V355 runtime-approved E2 Bank-B handler" if member == PSX else "V357 user-reviewed dialogue reinsertion"
    ...
    manifest = {
        "version": VERSION,
        "status": "STATIC CANDIDATE / USER-REVIEWED / RUNTIME PENDING / TEST_ONLY",
        "release_blocker": "cold-boot and representative runtime traversal remain pending",
        ...
        "input_sha256": {
            ...
            "targets": TARGETS_SHA256, "nontext": NON_TEXT_SHA256,
        },
    }
    ...
    report = [
        "Arc the Lad 1 V357 user-reviewed full dialogue Bank-B build",
        "status: STATIC CANDIDATE / USER-REVIEWED / RUNTIME PENDING / TEST_ONLY",
        f"base: {BASE.name}",
        ...
        "user review: 47 former Bank-B draft rows approved through the 2026-09-03 export",
    ...
    (ANALYSIS / "runtime_checklist.txt").write_text(
        "\n".join([
            "V357 TEST_ONLY runtime checklist",
            "1. Cold boot V357; do not resume a pre-V357 RAM snapshot directly.",
            ...
            "5. Spot-check the user-edited dialogue, including 7/S7022 0x488F0 (single speaker prefix).",
            "6. Save states for any visual, timing, cursor, freeze, or wording defect.",
            "",
        ]), encoding="utf-8"
    )
    print(f"V357 build complete: {full_path.name}")
