...
WORK = ROOT / "01_work" / "story_test_09_twobyte_mapc"
WORK_FONT = WORK / "COMM.IMG"
WORK_DAT = WORK / "1" / "S1071.DAT"
OUTPUT = ROOT / "03_output" / "story_test_09_twobyte_mapc_patch_only.zip"
...
def extended_index(first_byte: int, second_byte: int) -> int:
    return ((second_byte >> 2) << 6) + (first_byte >> 2) - 1


def position(index: int) -> tuple[int, int]:
    return (index % 21) * 12, (index // 21) * 12
...
def main() -> None:
    ...
    if WORK_FONT.exists() or WORK_DAT.exists() or OUTPUT.exists():
        raise SystemExit("Story test 09 already exists; refusing to overwrite it.")
    ...
    village_position = position(extended_index(0xDD, 0x0B))
    elder_position = position(extended_index(0xD4, 0x25))
    write_glyph(font, *village_position, "촌")
    write_glyph(font, *elder_position, "장")
    ...
    print("Village:", extended_index(0xDD, 0x0B), village_position)
    print("Elder:", extended_index(0xD4, 0x25), elder_position)
