...
OUT = ROOT / "03_output"
STEM = "arc1_v278_full_korean_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v278_full_korean"
COMM = "COMM.IMG"
...
def indices(b: bytes) -> set[int]:
    ...
    while i < len(b):
        ...
        if w == 1 and 0x01 <= t[0] <= 0xDC:
            out.add(t[0] - 1)
        elif w == 2 and 0xDD <= t[0] <= 0xE8 and 0x01 <= t[1] <= 0xFE:
...
def main() -> None:
    ...
    rep = [
        "v278 TEST ONLY - full translated script, original disc, 12px johab",
