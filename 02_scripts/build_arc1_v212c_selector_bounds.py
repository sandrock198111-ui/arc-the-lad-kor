"""Build v212c: bound the v212b OT selector before packaging.

v212b fixed the cross-cave continuation, but its compact selector only stopped
at link zero.  A PlayStation DMA chain may terminate with 0xFFFFFF, so reading
that value as KSEG0 would leave the 2 MiB RAM window.  This successor consumes
the two remaining entry NOPs and accepts only links 1..0x1FFFFF.
"""
...
OUT_STEM = "arc1_v212c_ab_cache_selector_bounded_TEST_ONLY"
...
def bounded_entry() -> bytes:
    ...
    words = [
        ...
        old.i_type(0x09, v212.T0, v212.T2, -1),
        old.r_type(v212.ZERO, v212.T2, v212.T2, 21, 0x02),
        v212.branch(0x05, v212.T2, v212.ZERO, e + 7 * 4, finish),
        ...
        v212.branch(0x05, v212.T6, v212.ZERO, e + 14 * 4, e + 18 * 4),
        ...
        v212.branch(0x04, v212.A2, v212.ZERO, e + 18 * 4, next_packet),
        old.i_type(0x0C, v212.T5, v212.T6, 0xFC),
        old.i_type(0x09, v212.T6, v212.T6, -0x64),
        v212.branch(0x05, v212.T6, v212.ZERO, e + 21 * 4, next_packet),
        v212.NOP,
    ...
    if len(blob) != v212.ENTRY_CAP:
        raise SystemExit("bounded selector entry size differs")
    return blob
...
def main() -> None:
    ...
    if not diffs or any(not entry_at <= i < entry_at + v212.ENTRY_CAP for i in diffs):
        raise SystemExit(f"v212c changed outside selector entry: {diffs[:20]}")
    ...
    notes = v212.validate_selector(v212.SELECTOR_ENTRY, bounded, ranges, frame)
    ...
    if refs != [(v212.SELECTOR_ENTRY + 30 * 4, "j", v212.SELECTOR_CHECK)]:
        raise SystemExit(f"bounded selector continuation graph differs: {refs}")

    # Exact arithmetic proof for the compact 1..0x1FFFFF range test.
    for link, accepted in ((0, False), (1, True), (0x1FFFFF, True),
                           (0x200000, False), (0xFFFFFF, False)):
        test = (((link - 1) & 0xFFFFFFFF) >> 21) == 0
        if test != accepted:
            raise SystemExit(f"selector link-bound proof failed at 0x{link:06X}")

    md = Cs(CS_ARCH_MIPS, CS_MODE_MIPS32 + CS_MODE_LITTLE_ENDIAN)
    decoded = list(md.disasm(bounded, v212.SELECTOR_ENTRY))
    if sum(item.size for item in decoded) != len(bounded):
        raise SystemExit("Capstone did not consume bounded selector entry")
    ...
    report = [
        "v212c TEST ONLY - bounded per-frame A/B cache selector",
        f"base={BASE.name}",
        f"base_sha256={BASE_SHA256}",
        f"output={output.name}",
        f"sha256={stamp}",
        "release_status=DIAGNOSTIC; DO NOT DISTRIBUTE",
        "selector_link_range=1..0x1FFFFF",
        ...
        f"selector entry 0x{v212.SELECTOR_ENTRY:08X} / {len(bounded)} bytes",
        ...
        *notes,
        ...
        "rollback=v210; do not use v211/v212/v212b",
    ]
    REPORT.write_text("\n".join(report) + "\n", encoding="utf-8")
    print("\n".join(report))
    print(REPORT)
