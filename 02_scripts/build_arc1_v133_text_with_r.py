"""v133: the text again, now that R and 씩 exist.

v132 put R into strip D slot 49 and 씩 into slot 50, so the lines that needed them --
17 uses of R, 6 of 씩 -- can finally be written. Nothing else about the build changed:
this is the same in-place text pass v130 and v131 ran, against the same CSV, with the
same guards. It rewrites only bodies it can prove out, keeps E5 choice bodies exactly
as they are, and refuses to finish if any glyph it wrote lands on the texture page the
classifier cannot reach or if any live slot loses its terminator.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v132_r_and_ssik_099D6D90.zip"
BASE_SHA = "099D6D90884FD6D2571154942379270DEF8AAFAA9197C7B0C6C98C9CEE992681"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v133_text_with_r"
ANALYSIS = ROOT / "01_work/analysis/arc1_v133_text_with_r"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v132")
    ...
    lines = [
        ...
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(rebuilt)}; PSX.EXE and COMM.IMG byte-identical to v132",
        "",
