...
STEM = "arc1_v261_restore_lookup_safe_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v261_restore_lookup_safe"
...
def main() -> None:
    ...
    # A destination cell must clear four tests, not one.  v260 checked only for
    # ink and overwrote 95 cells the script still points at.
    from audit_dynamic_cache_requirements import glyph_index as _gi
    reserved = v231.range_table_indices(members[PSX])
    referenced = set()
    for name, s0, e0 in v231.text_regions(before):
        d = before[name]
        i = s0
        while i < e0:
            b = d[i]
            if b == 0xE2 or 0xE3 <= b <= 0xE8:
                ...
                continue
            w = 1 if b < 0xDD else 2
            g = _gi(bytes(d[i:i + w]), tuple(lut_cur))
            if g is not None:
                referenced.add(g)
            i += w
    lut_used = set(lut_cur)
    occupied = set()
    ...
    free = [i for i in range(COLS * ROWS * PL)
            ...
            and i not in referenced and i not in lut_used]
    ...
    rep = [
        "v261 TEST ONLY - lookup glyphs restored into verified-safe cells",
