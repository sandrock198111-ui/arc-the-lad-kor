"""Original Rev1 -> V380 cumulative patch, roundtrip and reproducibility gates."""
import json,os,subprocess,tempfile
from pathlib import Path
from zipfile import ZipFile,ZipInfo,ZIP_DEFLATED
from package_arc1_v375_xdelta import hashes,TOOL,SOURCE,ROOT,TOOL_SHA
import build_arc1_v380_followup as build
TARGET=ROOT/'03_output/V380_FOLLOWUP.bin'
OUT=ROOT/'03_output/arc1_kor_v0.9.6_rev1'
BUNDLE=Path(str(OUT)+'.zip')

def main():
    raise SystemExit('V380 is an internal superseded candidate; use package_arc1_v381_xdelta.py for v0.9.6.')
    ...
    assert report['cpu']['water_temple']['return_value']==1 and len(report['applied'])==5
    assert len(report['cpu']['followup_v380']['captures'])==6
    work=Path(tempfile.mkdtemp(prefix='v380_xdelta_',dir=ROOT/'01_work'))
    ...
    verification=dict(version='v0.9.6',internal_build=380,source=src,target=tgt,patch=hashes(patch),
        roundtrip_exact=True,reencode_exact=True,unexplained_payload_changes=0,
        cumulative_base=379,dialogue_corpus_edits=5,ui_edits=1,
        captured_cpu_cases=6,original_pause_comparisons=3,runtime_verified=False,
    ...
    readme=(ROOT/'05_docs/v380_patch_readme.txt').read_text(encoding='utf-8').format(source_crc=src['crc32'],source_md5=src['md5'],target_sha=tgt['sha256'])
