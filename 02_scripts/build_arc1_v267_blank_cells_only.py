...
STEM = "arc1_v267_blank_cells_only_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v267_blank_cells_only"
...
def main() -> None:
    ...
    # No reclaiming.  "Unreachable" can only be judged from the 2878 dialogue
    # sites that are recorded, and the script has far more than that -- v266
    # reclaimed 364 cells on that basis and broke lines the list never saw.
    # Cells that are blank were never anyone's glyph, so only those are used.
    reclaim = []
    ...
    rep = [
        "v267 TEST ONLY - ceiling raised, blank cells only, nothing reclaimed",
