...
OUT = ROOT / "03_output"
STEM = "arc1_v282_with_ui_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v282_with_ui"
...
def main() -> None:
    ...
    # UI strings inside PSX.EXE, from the tables this project already recorded
    # (ui_full_v42 / ui_safe_v39 / ui_system_v39 / ui_world_name_v39).  Only the
    # 391 verified offsets -- v269 scanned the whole 0x78000..0x83000 range,
    # rewrote executable code as text and would not boot.
    ui_path = ROOT / "01_work/analysis/ui_string_sites.pkl"
    n_ui = 0
    if ui_path.exists():
        for off, ln in sorted(pickle.load(open(ui_path, "rb")).items()):
            lines.append((PSX, off, ln))
            n_ui += 1
    step(f"UI 문자열 {n_ui}곳 추가")
    ...
    rep = [
        "v282 TEST ONLY - 16px, plus the recorded UI string tables",
