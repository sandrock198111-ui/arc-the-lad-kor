...
STEM = "arc1_v249_reencode_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v249_reencode"
...
def main() -> None:
    ...
    assign, nxt = {}, 0
    for ch in drawable[:ONE_BYTE_MAX - 1]:
        assign[ch] = nxt
        nxt += 1
    blank = nxt
    nxt = ONE_BYTE_MAX
    ...
    for r in rows:
        ...
        for ch in kr:
            c = code(ch)
            if len(out) + len(c) > cap:
                truncated += 1
                break
            out += c
        out += bytes((SPACE,)) * (cap - len(out))
        buf[start:start + cap] = out
        wrote += 1
    ...
    report = [
        "v249 TEST ONLY - script re-encoded from the translation source",
        f"base={base.name}", f"output={out.name}", f"sha256={stamp}",
        "release_status=DIAGNOSTIC; DO NOT DISTRIBUTE",
        f"glyphs={len(assign)}  (one-byte {min(len(drawable), ONE_BYTE_MAX-1)}, "
        f"two-byte {max(0, len(drawable)-(ONE_BYTE_MAX-1))})",
