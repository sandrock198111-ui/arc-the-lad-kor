"""Build v235: drop the relocated cache one row, clear of the flight zero-fill.

v233 moved the cache to (999,420) and the relocation itself worked -- packets
read the new rectangle, glyph/scenery cross-contamination vanished.  What
remained was the flight scene wiping the new rectangle to zero, and v234's
attempt to answer that with per-frame re-upload destroyed the cache instead:
disabling the hit test at 0x801FF49C makes each glyph claim a fresh slot, so
owners filled with 4..10 duplicates out of 28 and the displaced glyphs went
missing (savestates HASH-A13992E51BF4BB39_1..6).

Measuring page 15,1 row by row across flight and dialogue savestates pins the
zero-fill's lower edge:

    y352..415   0 in both        nothing writes here
    y416..431   flight 28/116, dialogue 87/219    <- v233 sits here, gets wiped
    y432..447   identical in both (160/297)       <- survives the flight

The fill stops at y431.  Row 36 (y432..443) is one row below and outside it.
Across the 467-state union the game touches only 24 of the 252 halfwords in
x999..1019 there -- x1000..1006 and x1016..1023 are completely clear, with the
rest scattered as 1..4 rows at x999 and x1007..1015.

Trading a rectangle that is erased wholesale every flight for one that overlaps
scenery in 9.5% of its cells is the point of this build.  Everything else --
X, U, the row-40 logical system, the U bias block -- is exactly as v233 left it.
"""
...
OUT_STEM = "arc1_v235_cache_row36_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v235_cache_row36"
...
OLD_V, NEW_V = 224, 176      # y432 - 256
OLD_X, NEW_X = 961, 999
OLD_U, NEW_U = 4, 156
DELTA = NEW_U + ((NEW_V - OLD_V) << 8)          # U += 156, V -= 48
assert DELTA == 156 - 48 * 256 == -12132
assert 0 <= NEW_U + (v171.CACHE_CELLS - 1) * old.CELL <= 255   # no carry into V
assert 0 <= NEW_V + old.CELL <= 256
...
def main() -> None:
    ...
    struct.pack_into("<2H", exe, rect_at, NEW_X, 432)
    ...
    report = [
        f"base={BASE.name}",
        f"cache rect (961,480) -> (999,432); packet U +4 -> +156, V 224 -> 176",
