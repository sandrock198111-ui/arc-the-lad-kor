"""Verified original Rev1-to-V376 hotfix package, without game images."""
import json,os,subprocess,tempfile
from pathlib import Path
from zipfile import ZipFile,ZipInfo,ZIP_DEFLATED
from package_arc1_v375_xdelta import hashes,TOOL,SOURCE,ROOT,TOOL_SHA
import build_arc1_v376_hotfix as build
TARGET=ROOT/'03_output/V376_HOTFIX.bin'
OUT=ROOT/'03_output/arc1_kor_v0.9.2_rev1'
BUNDLE=ROOT/'03_output/arc1_kor_v0.9.2_rev1.zip'
...
def main():
    ...
    assert report['cpu']['water_temple']['return_value']==1 and len(report['dat']['applied'])==70
    work=Path(tempfile.mkdtemp(prefix='v376_xdelta_',dir=ROOT/'01_work'))
    patch=work/'arc1_kor_v0.9.2_rev1.xdelta';decoded=work/'verified.bin';repeat=work/'repeat.xdelta'
    ...
    verification=dict(version='v0.9.2',internal_build=376,source=src,target=tgt,patch=hashes(patch),
        roundtrip_exact=True,reencode_exact=True,unexplained_payload_changes=0,
        ui_defects_fixed=102,dat_bodies_fixed=70,event_rows_restored=24,
    ...
    readme=f'''아크더래드 1 한글 패치 v0.9.2 (V376 긴급 수정) — 일본판 Rev 1 전용

적용 방법
1. 일본판 Rev 1 원본 BIN에 동봉 xdelta를 적용하세요.
   기존 v0.9.1/한글패치 BIN에 덧씌우지 마세요.
2. 출력 이름을 arc1_kor_v0.9.2.bin으로 지정하고 동봉 CUE를 옆에 두세요.
...
'''
    files={patch.name:patch.read_bytes(),'arc1_kor_v0.9.2.cue':b'FILE "arc1_kor_v0.9.2.bin" BINARY\r\n  TRACK 01 MODE2/2352\r\n    INDEX 01 00:00:00\r\n',
