"""Verify every changed raw sector against declared DAT writes/timestamps."""
from pathlib import Path
import json,hashlib
from verify_arc1_v362_disc_delta import timestamp_bytes,check_form1
from verify_iso_layout import read_iso
ROOT=Path(__file__).resolve().parents[1];AN=ROOT/'01_work/analysis/v363_dialogue_trial'
old=ROOT/'03_output/V362_KOREAN_TITLE_TEST.bin';new=ROOT/'03_output/V363_DIALOGUE_TRIAL_TEST.bin'
...
print('PASS',len(changed),'changed sectors;',len(changed)*2,'EDC/ECC checks; only declared DAT writes and ISO timestamps')
