...
OUT = ROOT / "03_output"
STEM = "arc1_v308_cache_static_TEST_ONLY"
ANALYSIS = ROOT / "01_work/analysis/arc1_v308_cache_static"
...
def main() -> None:
    ...
    # The battle cursor samples x0..31, y128..159 directly by UV.  A 12x12 glyph
    # overlapping that rectangle corrupts it, and row 10 (y120..131) already
    # overlaps -- codex_notes 2026-07-14.
    def hits_cursor(i):
        c, _ = divmod(i, PL)
        col, row = c % COLS, c // COLS
        x0, y0 = col * CELL, row * CELL
        return x0 < 32 and 0 < x0 + CELL and y0 < 160 and 128 < y0 + CELL

    taken = {tget(s) for s in range(SLOTS) if tget(s) < CACHE_MARK}
    free = [i for i in range(CACHE_MARK)
            if encodable(i) and i not in taken and not hits_cursor(i)
    ...
    rep = [
        "v308 TEST ONLY - cached glyphs made resident, cursor rectangle excluded",
