"""Package isolated V359 cursor trial from original staging; no old image writes."""
...
stem='V359_CURSOR_LINES_TEST'
p.WORK=ROOT/'01_work/package_v359_cursor_lines'
p.FILES=p.WORK/'files'
p.STATE=p.WORK/'applied.json'
binary=ROOT/'03_output'/f'{stem}.bin'
cue=binary.with_suffix('.cue')
report=json.loads((b.AN/'report.json').read_text(encoding='utf-8'))
assert b.digest(b.OUT.read_bytes())==report['zip_sha256']
