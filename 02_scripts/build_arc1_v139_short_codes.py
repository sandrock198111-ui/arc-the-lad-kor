"""v139: the whole script again, with the short codes in play.

v138 moved 86 syllables into one-byte cells -- 47% of every Korean character written --
so most lines are now about two thirds the size they were. This is the same in-place
pass as v130, v133, v134 and v136, run once more so the shorter encoding actually
reaches the disc: lines that needed an external slot now fit where the Japanese sat,
and files that had run out of slots have room again.

Choice bodies are left to v140, which redoes them on top of this.
"""
...
BASE_ZIP = ROOT / "03_output/arc1_v138_one_byte_font_375BCC7F.zip"
BASE_SHA = "375BCC7F0D43447D88404D626ADB2ED24353851A381845C4FE6A77D915B39088"
...
OUT_DIR, OUT_STEM = ROOT / "03_output", "arc1_v139_short_codes"
ANALYSIS = ROOT / "01_work/analysis/arc1_v139_short_codes"
...
def main() -> None:
    if digest(BASE_ZIP.read_bytes()) != BASE_SHA:
        raise SystemExit("base archive is not v138")
    ...
    lines = [
        "v139 the whole script with the short codes",
        "",
        f"base    {BASE_ZIP.name}",
        f"output  {final.name}",
        f"        sha256 {stamp}",
        f"members {len(rebuilt)}; PSX.EXE and COMM.IMG byte-identical to v138",
        "",
