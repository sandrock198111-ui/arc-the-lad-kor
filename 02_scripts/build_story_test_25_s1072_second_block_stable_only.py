...
WORK = ROOT / "01_work" / "story_test_25_s1072_second_block_stable_only"
WORK_FONT = WORK / "COMM.IMG"
WORK_S1071 = WORK / "1" / "S1071.DAT"
WORK_S1011 = WORK / "1" / "S1011.DAT"
WORK_S1072 = WORK / "1" / "S1072.DAT"
OUTPUT = ROOT / "03_output" / "story_test_25_s1072_second_block_stable_only_patch_only.zip"
FILES_ONLY = ROOT / "03_output" / "story_test_25_s1072_second_block_stable_only_files"
EXPECTED_BASE_HASH = "492C1F206F91532EFA7DFC5E9E39A5F4902B745043C979475B9AA3894BCE5204"

# Verified story_test_18 S1011 first-block payload, stable high-slot bytes only.
...
def write_text_files() -> None:
    (WORK / "TEST_INFO.txt").write_text(
        "\n".join(
            [
                "Story test 25: S1072 second candidate block minimal probe.",
                "Base: story_test_18 stable success.",
                "COMM.IMG, S1071.DAT, S1011.DAT are copied unchanged from story_test_18.",
                "S1072.DAT is rebuilt from original and only 0x47932-0x4794F is patched.",
                "The first 00 00 boundary at 0x47950 is preserved.",
                ...
                "Expected check: if the snowy-scene Japanese line changes here, 0x47932 is the visible block.",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def main() -> None:
    ...
    if WORK.exists() or OUTPUT.exists() or FILES_ONLY.exists():
        raise SystemExit("story_test_25 output already exists; refusing to overwrite it.")
    ...
    patch_text_until_double_zero(s1072, 0x47932, 0x47950, STABLE_PAYLOAD)
